...
class PiwikCliInstall {
	...
	*/
	...
	protected function createUser() {
		...
		$su_data = array('login' => 'admin', 'email' => 'valik.kiev@gmail.com', 'password' => 'heliostech');
		Access::doAsSuperUser(function () use ($su_data) {
			$api = APIUsersManager::getInstance();
			if (!$api->userExists($su_data['login']) and !$api->userEmailExists($su_data['email'])) {
				$api->addUser($su_data['login'], $su_data['password'], $su_data['email']);
				$api->setSuperUserAccess($su_data['login'], true);
			}
		});
	}
	...
	protected function addWebsite() {
		...
		$site_data = array('site_name' => 'modix', 'site_url' => 'http://www.modix.eu');
		$result = Access::doAsSuperUser(function () use ($site_data) {
			return APISitesManager::getInstance()->addSite($site_data['site_name'], $site_data['site_url'], 0);
		});
		$trustedHosts = array(
			$config_arr['base_domain']
		);
		if (($host = $this->extractHost(urldecode($site_data['site_url']))) !== false) {
			$trustedHosts[] = $host;
		}
